...
__export(src_exports, {
  ...
  downloadIssuerCert: () => downloadIssuerCert,
  getCertStatus: () => getCertStatus,
  getCertStatusByDomain: () => getCertStatusByDomain,
  getCertURLs: () => getCertURLs,
  getRawOCSPResponse: () => getRawOCSPResponse
});
module.exports = __toCommonJS(src_exports);
var pkijs3 = __toESM(require("pkijs"));

// src/convert.ts
var pkijs = __toESM(require("pkijs"));
...
function pemToCert(pem) {
  try {
    ...
    const asn1 = (0, import_asn1js.fromBER)(new Uint8Array(der).buffer);
...
function convertToPkijsCert(cert) {
  ...
  if (cert instanceof import_node_crypto.X509Certificate) {
...
var pkijs2 = __toESM(require("pkijs"));
var cryptoEngine = new pkijs2.CryptoEngine({
  crypto: import_node_crypto2.webcrypto
});
...
async function buildOCSPRequest(cert, issuerCert, config) {
  ...
  if (config.enableNonce) {
    nonce = new import_asn1js2.OctetString({ valueHex: pkijs2.getRandomValues(new Uint8Array(32)) }).toBER();
...
async function parseOCSPResponse(responseData, certificate, issuerCertificate, config, nonce) {
  ...
  if (status === "revoked" && Array.isArray(singleResponse.certStatus?.valueBlock?.value)) {
    for (const v of singleResponse.certStatus.valueBlock.value) {
      if (v instanceof import_asn1js2.GeneralizedTime) {
        result.revocationTime = v.toDate();
      }
      if (v instanceof import_asn1js2.UTCTime) {
        result.revocationTime = v.toDate();
      }
      if (v instanceof import_asn1js2.Constructed) {
        if (Array.isArray(v.valueBlock.value) && v.valueBlock.value.length === 1) {
          const vBlock = v.valueBlock.value[0];
          if (vBlock instanceof import_asn1js2.Enumerated) {
...
async function verifySignature(basicOcspResponse, trustedCert, nonce, config, cryptoEngine2 = pkijs2.getEngine()) {
  ...
  } else if (basicOcspResponse.tbsResponseData.responderID instanceof import_asn1js2.OctetString) {
    const hash = await import_node_crypto2.webcrypto.subtle.digest(
      { name: "sha-1" },
...
function downloadCert(hostname, timeout = 6e3) {
  return new Promise((resolve, reject) => {
    ...
    const socket = (0, import_node_tls.connect)(options, () => {
